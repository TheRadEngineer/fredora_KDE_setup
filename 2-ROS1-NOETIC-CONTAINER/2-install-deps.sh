...
if is_done "phase01"; then skip "Phase 1 (bootstrap) done"; else
  phase "PHASE 1/19 — Bootstrap tools"
...
if is_done "phase03"; then skip "Phase 3 (profile) done"; else
  phase "PHASE 3/19 — Container profile"
...
if is_done "phase04"; then skip "Phase 4 (apt/pip) done"; else
  phase "PHASE 4/19 — Apt packages + pip"
  export DEBIAN_FRONTEND=noninteractive
  sudo apt-get update || {
    err "apt update failed"
    exit 1
...
if is_done "phase05"; then skip "Phase 5 (OpenCV) done"; else
  phase "PHASE 5/19 — OpenCV legacy symlinks"
...
if is_done "phase07"; then skip "Phase 7 (gRPC) done"; else
  phase "PHASE 7/19 — gRPC ${GRPC_VERSION}"
  cd "$BUILD_DIR"
  [[ ! -d grpc ]] && {
...
if is_done "phase08"; then skip "Phase 8 (MongoDB drivers) done"; else
  phase "PHASE 8/19 — MongoDB drivers"
  cd "$BUILD_DIR"
...
if is_done "phase11"; then skip "Phase 11 (Cartographer) done"; else
  phase "PHASE 11/19 — Cartographer ROS"
...
if is_done "phase12"; then skip "Phase 12 (MongoDB server) done"; else
  phase "PHASE 12/19 — MongoDB server"
...
if is_done "phase13"; then skip "Phase 13 (Node.js) done"; else
  phase "PHASE 13/19 — Node.js ${NODE_VERSION} via nvm"
  export NVM_DIR="$HOME/.config/nvm"
  if [[ ! -d "$NVM_DIR" ]]; then
...
# =============================================================================
# PHASE 14: Neovim (latest stable) + LazyVim
# Ubuntu 20.04 ships Neovim 0.4.x — LazyVim needs 0.9+.
# We install the latest stable binary from GitHub.
# Nerd Fonts must be on the HOST (Konsole renders fonts host-side).
# =============================================================================
if is_done "phase14"; then skip "Phase 14 (LazyVim) done"; else
  ...
  info "First 'nvim' launch will auto-install plugins + Treesitter (~30s)"
  info "Mason will auto-install LSPs on first file open (clangd, pyright, etc.)"
  info "NOTE: Nerd Fonts must be installed on HOST for icons to render in Konsole"
  mark_done "phase14"
fi
...
# =============================================================================
# PHASE 16: Clone repos via vcstool
# =============================================================================
if is_done "phase16"; then skip "Phase 16 (clone) done"; else
  phase "PHASE 16/19 — Clone workspace repos"
  [[ ! -f "$REPOS_YAML" ]] && {
...
# =============================================================================
# PHASE 17: CATKIN_IGNORE hardware + workspace rosdep
# =============================================================================
if is_done "phase17"; then skip "Phase 17 (CATKIN_IGNORE + rosdep) done"; else
  phase "PHASE 17/19 — CATKIN_IGNORE hardware + workspace rosdep"
...
# =============================================================================
# PHASE 18: Simulation config (environment variables)
# =============================================================================
if is_done "phase18"; then skip "Phase 18 (sim config) done"; else
  phase "PHASE 18/19 — Simulation config"
...
# =============================================================================
if is_done "phase19"; then skip "Phase 19 (npm) done"; else
  phase "PHASE 19/19 — npm install"
  export NVM_DIR="$HOME/.config/nvm"
  [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
  nvm use "$NODE_VERSION"

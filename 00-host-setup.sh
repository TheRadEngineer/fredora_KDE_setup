...
stage_5_browser() {
    ...
    banner "STAGE 5 — REMOVE FIREFOX, INSTALL ZEN"
...
# =============================================================================
stage_12_vscode() {
    if is_done "stage_12"; then banner "STAGE 12 — VS CODE (done — skipping)"; return; fi

    banner "STAGE 12 — VS CODE"

    step "Adding Microsoft repo"
    [[ -f /etc/yum.repos.d/vscode.repo ]] && skip "Already configured" || {
...
EOF
    ...
    rpm -q code &>/dev/null && skip "Already installed" || {
        run sudo dnf install -y code && ok "Installed" || warn "Failed"
    }

    info "Turn off telemetry: Settings > search 'telemetry' > set to 'off'"
    mark_done "stage_12"
}

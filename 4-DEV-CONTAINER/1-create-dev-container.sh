...
if [[ "$confirm" =~ ^[Nn]$ ]]; then
    ...
    info "  bash $STAGING_DIR/2-install-dev-deps.sh $HAS_NVIDIA"
    exit 0
fi

if ! distrobox enter "$CONTAINER_NAME" -- \
    bash "$STAGING_DIR/2-install-dev-deps.sh" "$HAS_NVIDIA"; then
    err "Dependency install hit an error."
    err "To resume:"
    err "  distrobox enter $CONTAINER_NAME"
    err "  bash $STAGING_DIR/2-install-dev-deps.sh $HAS_NVIDIA --resume"
fi
